    ...
    def handle(self):
        ...
        for proxy_ip_row in proxy_ip_group:
            ...
            if ProxyIpUtil.is_effective(ip_type, proxy_ip):
                self.dao.modify('update ' + VerifyProxyIp.__DB_TABLE_MAIN + ' set status = 1 where id = ' + is_str)
                print(1111)
            else:
                self.dao.modify('update ' + VerifyProxyIp.__DB_TABLE_MAIN + ' set status = 0 where id = ' + is_str)
                print(2222)
